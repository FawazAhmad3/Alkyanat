...
export const EntityCard: React.FC<EntityCardProps> = ({ entity, isRtl = false }) => {
  ...
  return (
    <div className={`group relative flex flex-col justify-between overflow-hidden rounded-2xl border border-zinc-800/80 bg-zinc-900/40 p-6 sm:p-8 transition-all duration-300 hover:border-zinc-700/80 hover:-translate-y-1 hover:shadow-[0_15px_40px_-15px_rgba(0,0,0,0.8)] ${isRtl ? 'text-right' : 'text-left'}`}>
      {/* Decorative Glow Overlay */}
      <div className={`absolute ${isRtl ? '-left-20' : '-right-20'} -top-20 h-40 w-40 rounded-full bg-gradient-to-br ${entity.accentColor} opacity-5 blur-[80px] group-hover:opacity-15 transition-opacity duration-300`} />
      ...
      <div>
        ...
        >
          {isRtl ? 'طلب تفاصيل الشراكة' : 'Request Partnership Details'}
          <Icons.ArrowRight className={`h-3.5 w-3.5 transition-transform duration-300 ${isRtl ? 'rotate-180 mr-2 group-hover:-translate-x-1' : 'ml-2 group-hover:translate-x-1'}`} />
        </a>
      </div>
    </div>
  );
