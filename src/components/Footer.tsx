...
export const Footer: React.FC<FooterProps> = ({ currentLang, onPageChange }) => {
  ...
  return (
    <footer className="bg-brand-navy-dark border-t border-white/[0.06] pt-16 pb-10 text-slate-400 relative overflow-hidden">
      ...
      <div className="absolute top-1/2 left-0 -translate-y-1/2 w-80 h-80 bg-brand-blue/5 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute right-0 bottom-0 w-96 h-96 bg-brand-indigo/5 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className={`grid grid-cols-1 lg:grid-cols-3 gap-10 mb-12 ${isRtl ? 'text-right' : 'text-left'}`}>
          ...
          <div className="space-y-6">
            ...
            <p className="text-xs leading-relaxed text-slate-450 font-medium max-w-sm">
              {footer.aboutText}
            </p>
            <div className={`flex gap-3 pt-2 ${isRtl ? 'justify-end' : ''}`}>
              {data.socialLinks.map((social: { platform: string; url: string }) => (
                <a
                  ...
                  className="p-3 rounded-xl bg-white/[0.02] border border-white/[0.08] text-slate-300 hover:text-brand-navy hover:bg-brand-blue hover:border-brand-blue hover:scale-105 transition-all duration-300 cursor-pointer shadow-md"
                ...
                {section.links.map((link) => (
                  <li key={link.label}>
                    <a
                      ...
                      className="text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-brand-blue transition-colors duration-200 flex items-center group cursor-pointer"
                    >
                      <span className={`h-1.5 w-1.5 rounded-full bg-slate-700 group-hover:bg-brand-blue transition-all duration-200 ${isRtl ? 'ml-2.5' : 'mr-2.5'}`} />
                      {link.label}
                    </a>
                  </li>
                ))}
          ...
          <div className="space-y-3">
            ...
            {/* Address Card */}
            <div className="flex gap-3.5 p-3.5 bg-white/[0.02] border border-white/[0.06] rounded-2xl hover:border-brand-blue/30 transition-all duration-300 shadow-sm group">
              <div className="p-2 bg-brand-bg-light text-brand-blue rounded-xl h-9 w-9 shrink-0 flex items-center justify-center group-hover:bg-brand-blue group-hover:text-brand-navy transition-all duration-300">
            ...
            <div className="flex gap-3.5 p-3.5 bg-white/[0.02] border border-white/[0.06] rounded-2xl hover:border-brand-blue/30 transition-all duration-300 shadow-sm group">
              ...
              <div className="flex flex-col">
                ...
                <a
                  ...
                  className="text-xs text-white font-extrabold hover:text-brand-blue mt-0.5 transition-colors"
                >
            ...
            {/* WhatsApp Card */}
            <div className="flex gap-3.5 p-3.5 bg-white/[0.02] border border-white/[0.06] rounded-2xl hover:border-emerald-550 transition-all duration-300 shadow-sm group">
              <div className="p-2 bg-emerald-950 text-emerald-400 rounded-xl h-9 w-9 shrink-0 flex items-center justify-center group-hover:bg-emerald-500 group-hover:text-brand-navy transition-all duration-300">
        ...
        <div className={`pt-8 border-t border-white/[0.06] flex flex-col md:flex-row items-center justify-between gap-4 ${isRtl ? 'md:flex-row-reverse' : ''}`}>
          ...
          <button
            onClick={scrollToTop}
            className={`flex items-center gap-2.5 text-xs font-black uppercase tracking-widest text-slate-400 hover:text-brand-blue transition-colors group cursor-pointer ${isRtl ? 'flex-row-reverse' : ''}`}
          >
            <span>{footer.backToTop}</span>
            <div className="p-2 bg-white/[0.02] border border-white/[0.08] rounded-xl group-hover:bg-brand-blue group-hover:text-brand-navy transition-all duration-350 shadow-md">
