...
export const Home: React.FC<HomeProps> = ({ currentLang, onLangChange }) => {
  ...
  return (
    <div className={`min-h-screen bg-white text-zinc-900 flex flex-col justify-between font-sans overflow-x-hidden ${isRtl ? 'text-right' : 'text-left'}`}>
      {/* Header */}
      ...
      <main className="flex-grow flex items-center justify-center pt-32 pb-20 relative">
        {/* Ambient glow */}
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(99,102,241,0.02),transparent_50%)] pointer-events-none" />
        
        <div className="text-center space-y-4 max-w-md px-4 relative z-10">
          <h2 className="text-2xl font-bold uppercase tracking-wider bg-gradient-to-r from-indigo-800 via-amber-600 to-amber-700 bg-clip-text text-transparent font-sans">
